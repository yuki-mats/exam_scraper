...
class ProgressOutputUiContractTests(unittest.TestCase):
    ...
    def test_partial_refresh_ui_uses_year_fields_and_two_simple_modes(self):
        ...
        self.assertNotIn('id="maintenance-group-progress-title"', html)
        self.assertIn('aria-label="範囲別の進捗"', html)
    ...
    def test_evaluation_dialog_explains_configured_concurrency_limit(self):
        javascript = APP_PATH.read_text(encoding="utf-8")
        html = INDEX_PATH.read_text(encoding="utf-8")
        ...
        self.assertIn("評価待ちを設定上限で連続評価", html)
    ...
    def test_validated_work_and_artifact_sync_have_separate_ui_states(self):
        ...
        self.assertIn("state.qualificationRuns.find((run)", javascript)
        self.assertIn("includes(state.listGroupId)", javascript)
        self.assertIn("const visibleRun = displayedQualificationRun();", javascript)
