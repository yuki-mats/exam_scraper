...
class ArtifactSynchronizerTests(unittest.TestCase):
    ...
    def test_runs_strict_law_validation_after_upload_dry_run(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            record_law_audit_version(root, group, "5.3")
            commands = []
    ...
    def test_mixed_law_audit_versions_validate_modern_without_blocking_legacy(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            record_law_audit_version(root, group, "5.3", questions=[modern])
...
"""
    ...
    def test_blocked_partial_rework_keeps_authoritative_preview_scope(self):
      ...
      || [\"failed\", \"interrupted\"].includes(run.status)
        ...
        self.assertIn('`延べ${questionWorkItemCount}工程判定`', javascript)
    ...
    def test_dialog_controls_have_matching_javascript_handlers(self):
        ...
        self.assertIn("function qualificationRunPreviewSignature", javascript)
        self.assertIn("signature !== state.qualificationRunDialog.previewSignature", javascript)
        self.assertIn('previewStatus === "loading"', javascript)
        self.assertIn('["human", "orchestration"].includes(preview.kind)', javascript)
        self.assertIn('"run-preview-group-summary"', javascript)
        self.assertIn("preview.questionWorkItemCount || preview.workItemCount", javascript)
    ...
    def test_detail_cache_requires_an_existing_entry_before_reuse(self):
        ...
        self.assertIn(
            'state.questionContentApiVersion >= 1 && state.auditView.readOnly',
            javascript,
        )
        self.assertIn(
            '? "question-content"',
