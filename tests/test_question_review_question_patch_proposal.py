...
class IsolatedQuestionPatchWorkspaceTests(unittest.TestCase):
    ...
    def test_prepare_rebases_latest_sibling_before_atomic_commit(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            relative = Path("output/sample/questions_json/group/patch.json")
            canonical = root / relative
            ...
            workspace = IsolatedQuestionPatchWorkspace.create(
                root,
                root / "output/question_review_console/prepared",
                qualification="sample",
                mutable_paths=[relative.as_posix()],
            )
            candidate_path = workspace.root / relative
            ...
            latest[1]["explanationText"] = "external-2"
            canonical.write_text(json.dumps(latest), encoding="utf-8")
            binding = SourceIdentityBinding.from_mapping(records[0])
            aliases = {relative.as_posix(): [list(binding.as_tuple())]}
            with workspace.canonical_transaction(workspace.changed_paths()) as tx:
                prepared = tx.prepare(binding=binding, aliases_by_path=aliases)
                before_commit = json.loads(canonical.read_text(encoding="utf-8"))
                staged = json.loads(candidate_path.read_text(encoding="utf-8"))
                committed = prepared.commit()
                after_commit = json.loads(canonical.read_text(encoding="utf-8"))

        self.assertEqual(before_commit[0]["explanationText"], "before-1")
        self.assertEqual(staged[0]["explanationText"], "candidate-1")
        self.assertEqual(staged[1]["explanationText"], "external-2")
        self.assertEqual(committed, [relative.as_posix()])
        self.assertEqual(
            [record["explanationText"] for record in after_commit],
            ["candidate-1", "external-2"],
        )
