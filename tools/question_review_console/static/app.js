...
function maintenanceLoadingWeights() {
  return state.maintenanceLoading.includeQuestions
    ? {
      connection: 10,
      workflow: 38,
      runs: 22,
...
function maintenanceLoadingStatusLabel() {
  const steps = state.maintenanceLoading.steps;
  if (Object.values(steps).includes("error")) return "最新情報を取得できませんでした";
  if (steps.apply === "active") return "画面へ反映しています";
  if (steps.questions === "active") return "問題一覧を確認しています";
  if (steps.workflow === "active" && steps.runs === "active") {
    return "フォルダと作業状況を確認しています";
  }
  if (steps.workflow === "active") return "フォルダ別の状態を確認しています";
  if (steps.runs === "active") return "実行中の作業を確認しています";
  if (steps.connection === "active") return "接続を確認しています";
  return state.maintenanceLoading.fallbackLabel || "更新を準備しています";
}

function renderMaintenanceLoadingProgress() {
  const weights = maintenanceLoadingWeights();
  let measured = 0;
  for (const [stepId, weight] of Object.entries(weights)) {
    const status = state.maintenanceLoading.steps[stepId];
    if (status === "done") measured += weight;
    else if (status === "active") measured += weight * 0.2;
  }
  state.maintenanceLoading.progress = Math.max(
    state.maintenanceLoading.progress,
    Math.min(100, Math.round(measured)),
  );
  const progress = state.maintenanceLoading.progress;
  const meter = $("#maintenance-loading-meter");
  meter.setAttribute("aria-valuenow", String(progress));
  $("#maintenance-loading-meter-value").style.width = `${progress}%`;
  $("#maintenance-loading-title").textContent = maintenanceLoadingStatusLabel();
}
...
function updateMaintenanceLoadingElapsed() {
  ...
  $("#maintenance-loading-elapsed").textContent = `${elapsedSeconds}秒`;
}
...
function startMaintenanceLoading({
  ...
  $("#maintenance-loading-meter").removeAttribute("aria-invalid");
  $("#maintenance-loading-message").hidden = true;
  $("#maintenance-loading-message").textContent = "";
  ...
  state.maintenanceLoading.includeQuestions = includeQuestions;
  state.maintenanceLoading.steps = Object.fromEntries(
    stepIds.map((stepId) => [
      stepId,
  ...
  state.maintenanceLoading.progress = 0;
  state.maintenanceLoading.fallbackLabel = title;
...
function failMaintenanceLoading(message, requestSequence) {
  ...
  $("#maintenance-loading-message").hidden = false;
  $("#maintenance-loading-elapsed").textContent = "停止";
...
function qualificationWorkflowCacheKey(qualification = state.qualification) {
  return `question-review-workflow:v${QUALIFICATION_WORKFLOW_CACHE_VERSION}:${qualification}`;
}

function cacheQualificationWorkflow(workflow) {
  if (!workflow?.qualification || !Array.isArray(workflow.groups)) return;
  try {
    localStorage.setItem(
      qualificationWorkflowCacheKey(workflow.qualification),
      JSON.stringify({
        version: QUALIFICATION_WORKFLOW_CACHE_VERSION,
        savedAt: Date.now(),
        workflow,
      }),
    );
...
function restoreCachedQualificationWorkflow() {
  if (!state.qualification) return false;
  try {
    const cached = JSON.parse(
      localStorage.getItem(qualificationWorkflowCacheKey()) || "null",
    );
    const workflow = cached?.workflow;
    if (
      !cached
      || cached.version !== QUALIFICATION_WORKFLOW_CACHE_VERSION
      || Date.now() - Number(cached.savedAt || 0)
        > QUALIFICATION_WORKFLOW_CACHE_MAX_AGE_MS
      || workflow?.qualification !== state.qualification
...
async function initialize() {
  ...
  try {
    ...
    const auditModelLabel = codexStatus.auditModel || modelLabel;
    const effortLabel = codexStatus.turnReasoningEffort || "標準";
    $("#project-status").textContent = state.evaluationEnabled
      ? `Codex: 作業 ${modelLabel}・再試行 ${retryModelLabel}・監査 ${auditModelLabel} / 推論 ${effortLabel} ・ ChatGPT ${codexStatus.planType} Standard ・ Firestore: ${session.projectId}`
      : `Codex App Server: 開始不可 ・ ${codexStatus.reason || "状態を確認できません"}`;
    $("#project-status").title = codexStatus.configuredReasoningEffort
      ? `Codex全体設定: ${codexStatus.configuredModel || "自動選択"} / 推論 ${codexStatus.configuredReasoningEffort} ・ 問題整備システム: 作業 ${modelLabel}・再試行 ${retryModelLabel}・監査 ${auditModelLabel} / 推論 ${effortLabel}`
      : "";
    initializeSelectors();
...
async function loadQualificationWorkflow(
  ...
  try {
    ...
    $("#maintenance-entry-guidance").replaceChildren(
      element("strong", "", "範囲を読み込めません"),
...
function renderQualificationWorkflow() {
  ...
  action.textContent = selectedStage.kind === "human" && selectedStage.action.type === "open_run"
    ? "対象範囲を一覧から選ぶ"
    : selectedStage.action.label;
...
function qualificationMaintenanceEntryStage() {
  const workflow = state.qualificationWorkflow;
  if (!workflow) return null;
  return workflow.stages.find((stage) => (
    stage.kind === "human"
    && stage.batchSelectable
    && stage.supportsGroupScope
    && stage.updateTargets?.length
  )) || null;
}

function openListGroupMaintenance(listGroupId) {
  const workflow = state.qualificationWorkflow;
  if (!workflow) return;
...
function returnToMaintenanceGroupList() {
  closeWorkflowGuide({ reopenRun: false });
  closeAuditView({ restoreFocus: false });
  const list = $("#maintenance-year-progress");
...
function renderMaintenanceDashboard() {
  ...
  const guidance = $("#maintenance-entry-guidance");
  guidance.replaceChildren(
    element("strong", "", "下の年度・フォルダから開始"),
  ...
  if (workflow.restartRequired) {
    ...
    guidance.replaceChildren(
      element("strong", "", "再起動後に開始できます"),
      element("span", "", "workflow設定と画面を同じ世代へ更新してください。"),
    );
  }
  if (isRunning) {
    ...
    guidance.replaceChildren(
      element("strong", "", checkingResult ? "最終検証中" : "整備を実行中"),
      element("span", "", "完了後に別の年度・フォルダを開始できます。"),
    );
  }
...
function renderWorkflowGuideContext() {
  ...
  action.textContent = stage.kind === "human" && stage.action.type === "open_run"
    ? "対象範囲を一覧から選ぶ"
    : stage.action.label;
...
function selectedQualificationRunMode() {
  return document.querySelector('input[name="qualification-run-mode"]:checked')?.value || "needed";
}
...
function qualificationRunSelectableUpdateTargets() {
  const categoryReady = state.qualificationWorkflow?.stages?.find(
    (stage) => stage.id === "category_setup",
  )?.status === "ready";
  return (state.qualificationWorkflow?.stages || [])
    ...
    .flatMap((stage) => (stage.updateTargets || []).map((target) => ({
      ...target,
      stageId: stage.id,
...
function qualificationRunUpdateTargets(stageIds = []) {
  if (state.qualificationRunDialog.fieldFirst) {
    return qualificationRunSelectableUpdateTargets();
  }
...
function qualificationRunStageIdsForUpdateTargetIds(targetIds = []) {
  const selected = new Set(targetIds);
  return (state.qualificationWorkflow?.stages || [])
    .filter((stage) => (stage.updateTargets || []).some(
      (target) => selected.has(target.selectionId),
    ))
    .map((stage) => stage.id);
}
...
function defaultQualificationRunUpdateTargetIds(stageIds, options = {}) {
  const selectedStages = new Set(stageIds);
  const available = (state.qualificationWorkflow?.stages || [])
...
function renderQualificationRunUpdateTargets(stageIds, selectedTargetIds) {
  ...
  for (const target of targets) {
    ...
    input.addEventListener("change", syncQualificationRunStagesFromUpdateTargets);
...
function syncQualificationRunStagesFromUpdateTargets() {
  ...
  if (state.qualificationRunDialog.fieldFirst) {
    ...
    const stage = qualificationWorkflowStage(state.qualificationWorkflowStageId);
...
function setQualificationRunUpdateTargetSelection(targetIds) {
  ...
  syncQualificationRunStagesFromUpdateTargets();
}

function selectAllQualificationRunUpdateTargets() {
  const targetIds = qualificationRunSelectableUpdateTargets()
    .map((target) => target.selectionId);
  setQualificationRunUpdateTargetSelection(targetIds);
}

function qualificationRunSupportsGroupScope(stage, stageIds = []) {
  if (state.qualificationRunDialog.fieldFirst) return true;
...
function renderQualificationRunGroups(stage, selectedGroupIds) {
  ...
  $("#qualification-run-group-refresh-label").textContent = `選択${scopeName}の全問題を洗い替える`;
  for (const group of groups) {
    const label = element("label", "");
    const input = document.createElement("input");
    input.type = "checkbox";
...
function renderQualificationRunStages(stage, selectedStageIds) {
  ...
  fieldset.hidden = state.qualificationRunDialog.simplified
    || state.qualificationRunDialog.fieldFirst
    || !stage.batchSelectable;
  container.replaceChildren();
  ...
  for (const item of selectable) {
    ...
    input.addEventListener("change", () => {
      ...
      } else if (supportsScope && refresh.checked) {
        groupRefresh.checked = true;
      }
...
function updateQualificationRunHeading() {
  ...
  if (!workflow) return;
  if (state.qualificationRunDialog.fieldFirst) {
    const groupIds = selectedQualificationRunListGroupIds();
    const scope = groupIds.length ? groupIds.join("・") : "対象範囲";
    const targetCount = selectedQualificationRunUpdateTargetIds().length;
    $("#qualification-run-title").textContent = `${scope}を整備・洗い替え`;
    $("#qualification-run-purpose").textContent = targetCount
      ? "選択した項目から必要工程を自動で決め、workflow順に一問ずつ検査・確定します。"
...
function openQualificationRunDialog(stage, options = {}) {
  ...
  refreshLabel.hidden = supportsScope;
  groupRefresh.disabled = !supportsScope;
  ...
  const defaultMode = options.mode || "needed";
  for (const node of document.querySelectorAll('input[name="qualification-run-mode"]')) {
  ...
  for (const question of state.questions) {
    ...
    head.append(
      ...
      ...(question.isLawRelated
        ? [element("span", "question-kind-badge", "法令")]
        : []),
    );
...
function renderPublicationStatus(question, publication) {
  const node = section(state.auditView.readOnly ? "現在の状態" : "公開前の状態");
  ...
  node.append(source);
  if (!state.auditView.readOnly) node.append(renderPipelineActions(question));
  return node;
}
