...
16a. true_falseの正しい選択肢は、選択肢自体が自己完結し、追加できる学習情報がない場合、解説が`正しい。`だけでも減点しない。`正しい。`の後が選択肢の全文再掲、語順・語尾だけを変えた表面的な言い換え、又は循環説明だけなら、簡潔さではなく解説品質の欠陥として要再整備にする。説明を続ける場合は、選択肢にない判断基準、具体例、比較、仕組み、時点又は根拠位置を含める。
16b. true_falseの間違いの選択肢は、`間違い。`だけでは合格にしない。まず、`正しくは、〜。`に相当する完全な訂正文として、成立する値、範囲、場合分け、主体、対象又は条件を不足なく示す。その後に理由と選択肢との差を説明する。用語の一般的な意味だけを説明し、訂正文の一部を欠く解説は要再整備にする。
16c. 抽象的な語句の比較は、最小限の具体例を使った方が一度で理解できる場合、その例を含めて採点する。完了時期や成立条件など、問題を読んだ利用者が最初に知りたくなる情報は基本解説に含める。基本解説と同じ内容を尋ねる補足は減点し、基本解説で完結していれば補足0件を適切とする。
17. 法令問題は出題時と現行法を区別し、条・項・号と基準日又はrevisionをlocatorへ含める。計算問題は一般式、数値の代入、途中計算、最終値、単位、丸めを確認し、独立した数式がflutter_math_fork対応の`\\[...\\]`又は`$$...$$`で正しく囲まれていることを確認する。数式内で文字サイズを変えず、横幅超過はアプリの横スクロールに任せる。
