...
_EXPLANATION_FIELD_RULES: dict[str, Any] = {
    "explanationText": {
        ...
        "description": (
            ...
            "間違いの選択肢は正しい内容と判断を分ける差を必ず説明し、正しい内容を"
            "先に示す。選択肢との差は「選択肢は〜としているため誤りである」のように"
            "具体的に書く。"
            "「〜としている点が誤り」「〜である点が誤り」は使わない。"
        ),
    },
    "suggestedQuestionDetailsByChoice": (
...
_LAW_REVISION_FACTS_RULE: dict[str, Any] = {
    "description": (
        ...
        "一次情報本文を確認済みでもXML locatorや改正IDを確認できない場合は、"
        "refs=[]、displayRefIds=[]とし、確認した根拠と未確認事項をexplanationText、"
        "differenceSummary、notesへ記録する。構造化できないrefsの値を推測しない。"
        "evidenceBindingHash等のserver-owned fieldはserverが付与する。refsのobjectは"
