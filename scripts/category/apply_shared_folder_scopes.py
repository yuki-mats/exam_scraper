...
def apply_folder_scopes(
    category: dict[str, Any],
    mapping: dict[str, Any],
    *,
    folder_scope_source: str | None = None,
) -> dict[str, Any]:
    ...
    note = (
        "folders.qualificationIds/licenseNames define which qualification "
        "divisions share each subject; questionSets/questions remain canonical "
        "shared records."
    )
    if isinstance(notes, list) and note not in notes:
        notes.append(note)
    if folder_scope_source:
        metadata["folderScopeSource"] = folder_scope_source
    return category


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Apply qualification/license arrays to canonical shared folders."
    )
    parser.add_argument("--category-json", type=Path, required=True)
    parser.add_argument("--mapping-json", type=Path, required=True)
    parser.add_argument(
        "--folder-scope-source",
        default=None,
        help="Metadata value written to metadata.folderScopeSource.",
    )
    ...
    category = apply_folder_scopes(
        load_json(category_path),
        load_json(mapping_path),
        folder_scope_source=args.folder_scope_source or str(mapping_path),
    )
    folders = category.get("folders", [])
    scoped = [
        {
