...
- 更新した場合の保存先と補足質問の採否は、上位READMEが示す共通field契約と工程03に従う。医師国家試験だけの補足質問必須ルールは設けない。
- 医師国家試験では最終成果物に `lawReferences` を入れないため、出題当時との差は基本解説の注記で伝える。補足質問は、基本解説にない追加情報を回答できる場合だけ作る。
