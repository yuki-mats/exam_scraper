...
公式上位分類は出題範囲を集計する軸、`folder` は学習範囲と進捗を確認する軸として扱う。両者は対応させるが、必ずしも1対1にする必要はない。

公式上位分類をそのまま `folder` にすると、一つの `folder` に多数の `questionSet` が集中し、進捗や苦手分野を把握しにくくなる場合がある。この場合は、公式の下位項目、専門家教材の章立て、繰り返し出題される論点に沿って、学習上意味のある複数の `folder` へ分割してよい。`folder` 数だけを先に決めず、各 `folder` に並ぶ `questionSet` の数と、一覧から学習状況を把握できるかを一緒に確認する。

分割は、問題数を均等にすることだけを目的に行わない。各 `folder` が独立した学習範囲として説明できることを優先し、細かく分けすぎて全体像を見失う構成も避ける。

公式上位分類を複数の `folder` へ分割した場合は、`category.json` の `metadata.officialCategoryMappings` に公式上位分類と `folderId` の対応を記録する。`isDeleted: false` の各 `folder` は一つの公式上位分類に対応させ、同じ `folderId` を複数の公式上位分類へ重複させない。これにより、問題紐付け後の問題数や進捗を公式上位分類へ再集計できる状態を保つ。詳しい分割根拠と境界は、資格固有の `03_category_preparation.md` に残す。この対応表はrepo内の分類・集計用であり、Firestoreへ自動反映されることを意味しない。
...
```json
{
  "metadata": {
    "officialCategoryMappings": [
      {
        "officialCategoryId": "official_01",
        "officialCategoryName": "公式上位分類名",
        "folderIds": [
          "example_f01_topic_a",
          "example_f02_topic_b"
        ]
...
- 問題数を均等にするためだけに、学習上の境界がないまま分割した分類
- 分割によって公式上位分類との対応を失い、元の出題範囲へ再集計できない分類
