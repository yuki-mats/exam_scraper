...
問題ごとに変更量を一律にせず、以下の変更前・変更後を目安にします。AWSサービス名、試験特有の用語、判断条件、数値、選択数、難易度、正答の根拠は維持します。シナリオ問題では、「企業は」「同社は」などの主語を先頭に置く公式翻訳の構造も維持します。
...
```text
EC2インスタンスが開発環境用として不定期に利用されている。利用期間中の数週間は安定稼働が必要だが、利用後はインスタンスを停止する。
...
```text
小売企業は、自社製品の評判を把握するため、SNS上の顧客コメントを分析したい。企業は、コメントをポジティブ、ネガティブ、あるいは中立に自動的に分類したい。この要件を満たすために最適なAWSサービスはどれか。
```

### 例5：責任共有モデルの主語を維持して表現を変える
...
```text
企業は、オンプレミス環境で運用していたリレーショナルデータベースをAmazon RDSへ移行した。移行後も同社が引き続き責任を負う管理タスクはどれか。
```
