...
- `lawReferences`を肢へ割り当てる前に、条文本文がその肢の主語、対象設備・制度、義務・禁止・例外を実際に規定しているかを確認する。法令名や条番号が存在するだけでは足りない。外側配列の位置、`choiceIndex=表示番号-1`、`lawContextForExplanation`内の選択肢番号、肢本文の四つを照合し、別の肢又は別制度の条文を流用しない。
