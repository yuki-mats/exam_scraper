...
5. mergeを再実行し、レビュー結果を `20_merged_1` に反映してから02b・03へ進む。
...
- 03bで正誤が後から変わった場合は`23_correctChoiceText_fixed`を更新し、merge後に03を再生成する。
...
`correctChoiceText`の型と値は`questionType`の契約に従います。保存後は、sourceとの件数・ID一致、値の型、再mergeした`20_merged_1`への反映を検証してください。
