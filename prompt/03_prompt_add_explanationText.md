...
法令問題では、原則として次の3段階を2〜3文で書く。

1. **誤りの特定**: 選択肢から判断を分ける語句・数値・条件を必要最小限で引用し、最初に何が誤りかを示す。
2. **条文根拠**: 正式法令名と正確な条文位置を示し、判断の決め手となる該当文を必要最小限で引用または要約する。
3. **正しい理解**: `したがって、AではなくBである` のように、選択肢と条文の差を対比して締める。

標準テンプレート:

```text
間違い。選択肢の「〔誤っている部分〕」が誤り。〔正式法令名〕第○条第○項第○号〔ただし書・表・別表の位置〕は「〔判断の決め手となる必要最小限の条文〕」と定めている。したがって、〔選択肢の内容〕ではなく〔正しい内容〕である。
```
...
```text
間違い。選択肢の「その使用を一時停止すべきこと」が誤り。ガス事業法第161条は「消費機器を修理し、改造し、又は移転すべきこと」を命ずることができると定めている。したがって、一時停止ではなく、修理・改造・移転によって技術上の基準へ適合させる命令である。
```

#### 誤りタイプ別の書き分け

誤りの性質に応じて、標準構造の3段階目を次のように書き分ける。

1. **語句・主体・対象の置換**
   - `選択肢の「A」が誤り。○○法第X条第Y項は「B」と定めている。したがって、主体（対象）はAではなくBである。`
   - 誰が義務を負うか、誰に通知するか、何が対象かを省略しない。
2. **数値・期間・回数・閾値**
   - `選択肢の「6年に1回以上」が誤り。○○省令第X条第Y項の表は、当該区分を「1年に1回以上」と定めている。したがって、6年ではなく1年である。`
   - 数値だけでなく、その数値が適用される区分・条件も書く。
3. **原則・例外・ただし書**
   - `選択肢は例外を○○の場合としている点が誤り。○○法第X条第Y項本文はAを原則とし、同項ただし書はBの場合だけ「この限りでない」と定めている。したがって、例外となるのは○○ではなくBの場合である。`
   - 本文だけ、またはただし書だけを抜き出さず、正誤判断に必要なら原則と例外条件をセットで示す。
4. **義務・禁止・許可・裁量の強さ**
   - `選択肢の「しなければならない」が誤り。○○法第X条第Y項は「することができる」と定めている。したがって、義務ではなく裁量である。`
   - `できる`、`しなければならない`、`してはならない`、`努めなければならない` を混同しない。
5. **手続・時点・届出先**
   - `選択肢の「事前に経済産業大臣へ届け出る」が誤り。○○法第X条第Y項は、変更後に遅滞なく所轄産業保安監督部長へ届け出ると定めている。したがって、時点は事前ではなく変更後、届出先は大臣ではなく所轄産業保安監督部長である。`
   - 時点と提出先の両方が違う場合は、一方だけ説明して終わらせない。
6. **複数要件・列挙・接続関係**
   - `選択肢の「A又はB」が誤り。○○法第X条第Y項はAかつBを要件としている。したがって、いずれか一方では足りず、両方を満たす必要がある。`
   - 条文の `かつ` / `又は`、列挙の全部 / 一部、必要条件 / 十分条件を明示する。
7. **表・別表・区分対応**
   - `選択肢は区分Aに数値Cを対応させている点が誤り。○○省令第X条第Y項の表の区分Aの欄は数値Bと定めている。したがって、区分Aに対応するのはCではなくBである。`
...
- 法令問題の間違い解説が、原則として `誤り部分 → 正式法令名と条文位置 → 該当文 → AではなくB` の順になっているか
