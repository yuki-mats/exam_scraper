...
```text
正しい。〔規制対象・用語〕は、〔正式法令名〕第○条第○項において「〔判断の決め手となる必要最小限の条文〕」と定められており、選択肢の内容と一致する。
```

原則とただし書の双方が正しい選択肢では、本文だけでなく例外条件まで一致していることを書く。例えば、次のように説明する。

```text
正しい。消費機器の調査義務は、ガス事業法第159条第2項本文に定められている。同項ただし書は「その所有者又は占有者の承諾を得ることができないとき」を例外としており、選択肢は本文とただし書の双方に一致する。
```
...
誤り部分は、受験者が選択肢へ視線を戻さなくても違いを特定できる最小単位で引用する。語句だけで関係が分かりにくい場合は、`正しくはAがBにCする。選択肢はDがBにCするとしており、主体が異なるため誤りである`のように、主体・行為・対象の関係をまとめて示す。
