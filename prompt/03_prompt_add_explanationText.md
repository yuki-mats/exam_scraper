...
- `flash_card`と`group_choice`は、選択肢数にかかわらず問題共通の基本解説を`explanationText`の1要素だけに保存する。選択肢ごとの基本解説は作らない。
- `flash_card`は、正答へ至る考え方と正答選択肢の対応を一続きで説明する。
- `group_choice`は、正答と、比較・組合せ・対応関係を判断する基準を一続きで説明する。誤答選択肢ごとの独立した説明へ分けない。
...
- `true_false`の正しい選択肢は、判断基準、誤解しやすい境界又は仕組みなど、選択肢にない学習情報を追加できる場合だけ、その内容を書く。追加できる内容が選択肢の再掲又は表面的な言い換えしかない場合は、`正しい。`だけで終える。
- `true_false`の間違いの選択肢は、必ず正しい内容と判断を分ける差を書く。`間違い。`だけでは完了にしない。
- 独自問題の解説は、取得元の解説を材料に、05で維持した正答理由と各誤答の理由を分かりやすく説明する。
- `正しい。`の後に選択肢をそのまま再掲したり、語順・語尾だけを変えて言い換えたりしない。資格別文書が判断軸の説明を推奨していても、新しい情報がない解説を無理に付け足さない。
- `設問の通りです`、`記述のとおりです`、`正解だから正しい`のような循環説明を書かない。
...
正しい選択肢に追加の学習情報がある場合は、次のように続けます。

```text
正しい。選択肢にはない判断基準となる定義、条件、境界又は仕組みを簡潔に示す。
```
...
- 正しい選択肢の解説で、選択肢の全文又は表面的な言い換えを繰り返していないか。追加情報がなければ`正しい。`だけになっているか。
- 間違いの選択肢の解説で、正しい内容と判断を分ける差が省かれていないか。
...
| --- | --- |
| `true_false` | 間違いの選択肢は結論と判断を分ける条件を書く。正しい選択肢は追加の学習情報がある場合だけ説明を続け、なければ`正しい。`だけにする。 |
...
5. 正しい選択肢の解説が選択肢の全文又は表面的な言い換えを再掲していない。追加情報がなければ`正しい。`だけであり、間違いの選択肢は`間違い。`だけで終わっていない。
