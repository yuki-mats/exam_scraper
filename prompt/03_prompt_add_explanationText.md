...
この2要素で、なぜ間違いか、正しく表現するとどうなるかが理解できれば十分である。条文根拠までで差が明確ならそこで終え、差を言い換えた方が理解しやすい場合は、`したがって、AではなくBである` と補う。
...
```text
間違い。選択肢の「機能が失われた場合に保安を維持する」が誤り。ガス工作物の技術上の基準を定める省令第21条は、「停電等により当該設備の機能が失われることのないよう適切な措置を講じなければならない」と定めている。
```
...
3. **原則・例外・ただし書**
   - `選択肢は例外を○○の場合としている点が誤り。○○法第X条第Y項本文はAを原則とし、同項ただし書はBの場合に「この限りでない」と定めている。`
...
一つの選択肢に複数の誤りがある場合は、正誤を決める主要な誤りを先に説明する。学習上重要な誤りが複数ある場合は、それぞれの選択肢の文言と対応する条文根拠を順に示す。
...
法令問題では、選択肢の誤り部分と、条文中の判断を分ける部分を同じ説明内で対比する。これにより、条文名・番号・判断根拠が一つの流れで読める。
...
- `間違い。` の後に、判断を分ける選択肢の文言が続いている。
- 正式法令名と条文位置の後に、条文が何を定めているかが続いている。
