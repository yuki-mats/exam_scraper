...
選択肢の「誤っている語句・条件・数値・主体・対象範囲・順序・対応関係」が誤り。なぜ誤りなのか、正しくはどのような内容なのかを1〜2文で書く。
```
...
選択肢の「○○である」が誤り。△△の場合は○○ではなく□□と扱われるため、正しくは「□□である」。
```
...
1. **誤りの特定**: 選択肢から判断を分ける語句・数値・条件を必要最小限で引用し、最初に何が誤りかを示す。
2. **条文根拠**: 正式法令名と正確な条文位置を示し、判断の決め手となる該当文を必要最小限で引用または要約する。

この2要素によって、なぜ間違いか、正しく表現するとどうなるかが理解できれば十分である。条文の引用または要約だけでは選択肢との差が分かりにくい場合に限り、`したがって、AではなくBである` などの補正文を加える。補正文を毎回定型的に付けてはいけない。

標準テンプレート:

```text
間違い。選択肢の「〔誤っている部分〕」が誤り。〔正式法令名〕第○条第○項第○号〔ただし書・表・別表の位置〕は「〔判断の決め手となる必要最小限の条文〕」と定めている。
```

必要な場合だけ、末尾に次を加える。

```text
したがって、〔選択肢の内容〕ではなく〔正しい内容〕である。
```

通常は `設問文` ではなく `選択肢` と書く。各 `explanationText` が説明する対象は `choiceTextList` の一つだからである。`questionBodyText` 自体の記述を判定する形式に限り、`設問文の「〜」が誤り` と書いてよい。

誤り部分の引用は、受験者が選択肢へ視線を戻さなくても違いを特定できる長さにする。ただし、選択肢全体を引用し直さない。短い語句だけでは関係が分からない場合は、`選択肢は「AがBにCする」としている点が誤り` のように、主体・行為・対象の関係をまとめて示す。

具体例:

```text
間違い。選択肢の「その使用を一時停止すべきこと」が誤り。ガス事業法第161条は「消費機器を修理し、改造し、又は移転すべきこと」を命ずることができると定めている。
```
...
1. **語句・主体・対象の置換**
   - `選択肢の「A」が誤り。○○法第X条第Y項は、主体（対象）を「B」と定めている。`
   - 誰が義務を負うか、誰に通知するか、何が対象かを省略しない。
2. **数値・期間・回数・閾値**
   - `選択肢の「6年に1回以上」が誤り。○○省令第X条第Y項の表は、当該区分を「1年に1回以上」と定めている。`
   - 数値だけでなく、その数値が適用される区分・条件も書く。
3. **原則・例外・ただし書**
   - `選択肢は例外を○○の場合としている点が誤り。○○法第X条第Y項本文はAを原則とし、同項ただし書はBの場合だけ「この限りでない」と定めている。`
   - 本文だけ、またはただし書だけを抜き出さず、正誤判断に必要なら原則と例外条件をセットで示す。
4. **義務・禁止・許可・裁量の強さ**
   - `選択肢の「しなければならない」が誤り。○○法第X条第Y項は「することができる」と定めている。`
   - `できる`、`しなければならない`、`してはならない`、`努めなければならない` を混同しない。
5. **手続・時点・届出先**
   - `選択肢の「事前に経済産業大臣へ届け出る」が誤り。○○法第X条第Y項は、変更後に遅滞なく所轄産業保安監督部長へ届け出ると定めている。`
   - 時点と提出先の両方が違う場合は、一方だけ説明して終わらせない。
6. **複数要件・列挙・接続関係**
   - `選択肢の「A又はB」が誤り。○○法第X条第Y項は、AかつBを要件としている。`
   - 条文の `かつ` / `又は`、列挙の全部 / 一部、必要条件 / 十分条件を明示する。
7. **表・別表・区分対応**
   - `選択肢は区分Aに数値Cを対応させている点が誤り。○○省令第X条第Y項の表の区分Aの欄は数値Bと定めている。`
   - `表にある` だけで終わらせず、どの行・欄・区分を比較したかを書く。

一つの選択肢に複数の誤りがある場合は、正誤を決める主要な誤りを先に説明する。学習上重要な誤りが複数ある場合は列挙してよいが、条文の要約を並べるだけにせず、それぞれを選択肢の文言と対応させる。

周辺制度、類似論点、覚え方、例外の詳細など、選択肢の正誤理由を理解するために必須でない補足は `explanationText` へ詰め込まない。短い質問を `suggestedQuestions` のチップにし、対応する説明を `suggestedQuestionDetails` に入れる。ただし、選択肢がなぜ間違いか、正しく表現するとどうなるかという核心は、チップを開かなくても `explanationText` だけで完結させる。
...
法令問題では、根拠を確認できた場合でも条文名・番号を置くだけでは不十分である。選択肢の誤り部分と、条文中の判断を分ける部分を同じ説明内で対比する。

`lawReferences` を出す資格では、解説本文の法令名・条文位置・引用内容と、対応する `lawReferences` を一問一問照合する。アプリ内の条文遷移先が、説明で根拠にした条文と異なる状態を残してはいけない。
...
- `間違い。 記述は誤りです。`
- `間違い。 条件が違います。`
- `間違い。 ○○法第10条に規定されているため誤りです。`
- `間違い。 ○○法第10条はAと定めている。Bとしている点が誤り。`（誤り箇所が最後まで分からない）
- 条文を長く引用した後に、選択肢のどの文言と食い違うかを書かない説明
- `正しい。 覚えておきましょう。`
- `正しい。 正解です。`
- `正しい。 組合せが正しいです。`
- `正しい。 記述は正しい内容です。`
...
- 既存パッチを流用して修正する場合も、各選択肢について次を確認すること
  ...
  - 法令問題の間違いの選択肢で、誤り部分が条文説明より先に明示されているか
...
- 法令問題の間違い解説が、原則として `誤り部分 → 正式法令名と条文位置 → 該当文` の順になり、それだけで正しい内容まで理解できるか。必要な場合だけ `AではなくB` の補正文を加えているか
