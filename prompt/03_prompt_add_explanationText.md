...
用語を選ぶ問題は、各用語の意味と見分け方を問題共通の解説に含めます。計算問題は、式、代入値、単位換算、途中計算、最終値、正答との対応を順に示します。式は`flutter_math_fork`で表示できるよう、独立した数式を`\[...\]`、文中の短い数式を`\(...\)`で囲みます。数式内で文字サイズを変えず、画面幅を超える式はアプリの横スクロールに任せます。
...
計算問題は、文章で同じ換算を繰り返さず、数式だけで導出を追える形にします。

```text
間違い。正しくは、電界の強さは電圧を電極間の距離で割って求める。
\[
\begin{aligned} E &= \frac{V}{d} \\ &= \frac{100}{1 \times 10^{-3}} \\ &= 1 \times 10^{5}\ \mathrm{V/m} \end{aligned}
\]
```
...
- 計算問題は、解説だけで計算過程と答えを再現でき、数式の区切りと構文が正しい。
