...
4. 元の出題意図と資格固有の語感を保ち、問題文、選択肢又は選択肢の順番のうち、独自化に必要な部分だけを自然に変える。試験で意味を持つ用語、条件、数値、設問形式は動かさず、別問題へ作り替えない。問題文を維持して選択肢だけを変えてもよい。
5. `questionIntent`、`correctChoiceText`、`answer_result_text`を変更後の問題文と選択肢に合わせ、文章と正答を確定する。選択肢を並べ替える場合も、各選択肢と正誤の対応を維持する。画像生成前に、画像なしの05 patchとしてここまでを保存できる。
