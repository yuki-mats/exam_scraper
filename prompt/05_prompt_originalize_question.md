...
3. 問題文全体を別問題へ作り替えず、取得元固有の場面、固有名詞、数値、条件、情報順序又は表現から、その問題に適した一部を選んで意味のある変更を加える。必要な専門用語と、変更不要な箇所は維持する。選択肢はすべて維持してよい。変更が必要な場合も、一つ又は必要最小限の選択肢だけを調整し、正答の決め手、誤答としてのもっともらしさ、難易度を変えない。
4. `questionIntent`、`correctChoiceText`、`answer_result_text`を変更後の問題文と選択肢に合わせ、文章と正答を確定する。事実の訂正が必要な場合を除き、問い方、正答位置、受験者に要求する判断は取得元から変えない。画像生成前に、画像なしの05 patchとしてここまでを保存できる。
