...
複数選択肢のpatchでは、`lawRevisionFacts`を選択肢順の`list[object]`として保存し、各要素の`current.correctChoiceText`を対応するトップレベル`correctChoiceText`と一致させます。互換入力としてquestion-level objectを使う場合は、`current.correctChoiceText`を選択肢と同じ順序・件数の配列にします。`"第4肢が誤り"`のような問題全体の要約文字列は判定値にしません。正誤を変更しない場合もこのfieldは省略できません。
...
- `lawRevisionFacts.current.correctChoiceText`が全選択肢にあり、トップレベル`correctChoiceText`と順序・件数・判定が一致するか。`law_audit_metadata_incomplete`又は`law_audit_verdict_mismatch`が残る間はno-opの成功receiptを保存しない
