...
- 取得サイトが過去問の数値又は文言を現行法へ現在化している場合は、現在化後の`choiceTextList`へ出題時法令をそのまま当てて、公式元正答との矛盾を作らない。現在化前後の完全な文言、変更日、出題時と現行の一次法令及び公式元正答を独立に確認し、出題時原問題の正誤を`examTimeDecision`、現在化後の表示文言の正誤を`currentLawDecision`へ分ける。現在化前の文言を一次資料又は明示的な変更履歴から一意に確認できない場合は推測せず`hold`にする。
