...
6. 異なる場合は `needs_tertiary_review` として三次確定へ回す。
7. 同じ場合でも、年次監査 sidecar に `same_as_current` として記録する。
8. 二次監査では一次で取得した evidence bundle を使い、正答・解説・差分説明の妥当性を確認する。根拠不足なら `hold` とし、追加取得キューへ戻す。
9. 三次確定済みの `updated_to_current_law` の場合だけ、正誤更新 patch と解説更新 patch を公開確定する。
10. 未確定の場合は `hold` にし、推測更新しない。
...
- `quality-gate --require-law-grounded-flag --require-law-revision-facts --require-law-evidence-utilization` が通る形になっているか
