...
文字数、記号、数値、選択肢数ではなく回答操作で決めます。例えば、計算結果を問題文の条件から導ける問題は`flash_card`であり、数値候補であることだけを理由に`group_choice`にはしません。計算で一つの数値を求め、その値に最も近い数値候補を選ぶ問題も、受験者が導く答えは一つなので`flash_card`です。反対に、選択肢にしかない条件を比較しなければ答えられない問題は`group_choice`です。
