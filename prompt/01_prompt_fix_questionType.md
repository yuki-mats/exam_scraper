...
- 「2つ選択」「該当するものをすべて選択」のように、複数の独立した選択肢を正答として選ぶ問題は、各選択肢を個別に判定する`true_false`とする。`group_choice`は複数選択形式ではなく、選択肢群から正答を1つだけ選ぶ形式である。
- 現在の`correctChoiceText`は正答精査前の情報である可能性がある。正答数だけで分類せず、問題文と選択肢が求める回答体験から`questionType`を決める。
...
- **Step 2: `true_false` 判定を行う。**
  ...
  - 複数の選択肢を正答として選ぶ設問も、各選択肢が独立した記述であれば`true_false`とする。
...
- `group_choice`は選択肢群から1つだけ選ぶ回答体験であり、「複数の正答を選ぶ」という意味では使わない。
