...
計算を含むことだけで`flash_card`にはしません。問題文の条件だけでは具体的な答えを確定できず、`choiceTextList`に並ぶ各候補を計算又は比較して、その候補が条件を満たすかを個別に判定する問題は、各候補の判定が学習単位なら`true_false`です。例えば、複数の反応式から必要量が最大又は最小となる式を判定する問題で、`choiceTextList`が組合せ番号ではなく比較対象の反応式そのものを持つ場合は、各反応式の該当・非該当を判定する`true_false`とします。最終回答となる組合せ表そのものが`choiceTextList`に並ぶ場合だけ、その組合せ候補を比較する`group_choice`です。
