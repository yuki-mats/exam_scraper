...
`output/2nd-class-kenchikushi/questions_json/85010/20_merged_1/question_85010_1_merged.json` の 25 設問について、対象資格の専門家・問題作成者・参考書著者の観点で 1 問ずつ精査し、`correctChoiceText` を 99.99% 水準で確認する。あわせて、対応する `21_explanationText_added/question_85010_1_merged_explanationText_added_20260404_202312_01.json` の `explanationText` が教材として公開できる品質であることを確認し、必要なら patch を更新する。
...
`85010/question_85010_1_merged.json の 25 設問すべてについて、一問ずつの専門家レビュー、必要な裏取り、正しい回答の確認、教材品質の解説確認、必要な patch 更新、coverage check、内容監査が完了し、最終監査 receipt が full_outcome_complete: true を記録する。`
...
今回の tranche は `2nd-class-kenchikushi / 85010 / question_85010_1_merged.json` の 25 設問だけを扱う。最初の active Worker は `original_question_id=da6a8179822b27d9` とし、以後は同じファイル内の `question_bodies[]` 順に 1 問ずつ進める。
...
- `questionIntent` は `20_merged_1/question_85010_1_merged.json` を一次情報にし、不足時のみ `00_source/` を最小限参照する。
...
`docs/goals/nikyu-kenchikushi-expert-review-85010/state.yaml`
...
```text
/goal Follow docs/goals/nikyu-kenchikushi-expert-review-85010/goal.md.
```
...
8. Finish only with a final Judge/PM audit that proves all 25 target questions are done and meet the oracle.
