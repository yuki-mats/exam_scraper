...
現行の範囲選択、receipt、transaction、rollback、再開、`artifactSync`を保ち、範囲コンテナと一問単位のwork itemを分離する。modelはtoken量に応じた複数問のread-only候補生成だけを並列実行し、serverが問題別の検査、patch・工程版の確定、永続checkpointを所有する。一問の失敗は依存後続だけを止め、通常問題を先に完了してから失敗問だけを再実行する。最後に自動テストと再起動readbackを通し、問題整備UIからガス主任技術者甲種2019年を実行して実ログと成果物を確認する。
...
- modelは共有成果物を書き換えず、複数問の構造化候補だけを返す。serverがtoken量と実行状況からbatchと同時turn数を調整する。
- serverだけがexact identity単位で候補を検査し、合格した一問のpatch・工程版・checkpointを同じtransactionで確定する。
...
最初のWorkerは、状態ファイルだけ、queueだけ、UIだけに分割せず、「複数問のread-only候補生成、serverによる一問ずつの検査・確定・checkpoint、失敗後の独立問継続、再起動後の未確定問だけの再開」を一つの縦断sliceで実装する。成果物同期とUIも同じ安全境界へ統合する。
