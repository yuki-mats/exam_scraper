...
## Follow-up Tranche: 洗い替え条件の簡素化

2026-07-21の利用者合意に基づき、listGroupIdから開く整備画面を、運用者が迷わず使える三つの指定へ整理する。

- 指定する内容は「対象年度」「整備する項目」「処理する問題」の三つだけとする。
- 対象年度の複数選択は現状を維持する。
- 整備する項目は初期状態ですべて選択し、操作は「すべて選択」と「選択解除」に統一する。
- 問題番号の範囲指定はUIから削除する。既存runの読込互換は維持する。
- 処理する問題は「整備が必要な問題だけ」と「選択年度の全問題を洗い替える」の二択にする。
- 「整備が必要な問題」は、未整備、整備基準の更新、要確認をシステム側でまとめて判定する。利用者へ内部状態の選択を求めない。
- 旧run modeは既存runとAPIの読込互換のため保持してもよいが、新しい整備画面には表示しない。

## Follow-up Tranche: 問題状況の閲覧と絞り込み

2026-07-21の利用者合意に基づき、トップの各`listGroupId`行から、現在の整備内容を問題単位で確認できる読取専用の導線を追加する。

- `整備・洗い替え`の左に`状況を確認`を置き、タップした年度又はフォルダの全問題を表示する。
- 一覧は問題検索に加え、`00_source`と現在の正答が異なる問題、計算問題、法令問題をそれぞれ絞り込めるようにする。条件は併用できる。
- 問題詳細では、問題文、選択肢、現在の正誤、基本解説、選択肢別の補足質問、`questionType`、`isCalculationQuestion`、`00_source`との正答差分を確認できるようにする。
