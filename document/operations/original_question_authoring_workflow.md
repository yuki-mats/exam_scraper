...
問題文と選択肢は局所的な微修正にとどめ、変更後の内容を取得元の正答と解説に照らします。変更前後で正答、各選択肢の正誤、正答と誤答を分ける理由を維持します。後続の解説工程は、取得元の解説を材料として参照し、共通の`prompt/03_prompt_add_explanationText.md`に沿ってより分かりやすい独自解説へ再構成します。取得元の正答又は解説が公式資料と矛盾する疑いがある場合は、独自問題化で辻褄を合わせず、正答確認へ戻します。
...
- 複数問から一般化できた資格固有の出題構造は`prompt/qualification_docs/<qualification>/01_exam_profile.md`へ反映する。解説方針は資格別に作らず、`prompt/03_prompt_add_explanationText.md`の共通パターンを使う。
