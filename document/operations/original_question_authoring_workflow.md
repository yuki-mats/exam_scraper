...
独自問題化では、元の問題が持つ学習上の質をできる限り保ち、変更を必要最小限にします。問題文全体を別問題へ作り替える必要はありません。取得元に固有の場面、固有名詞、数値、条件、情報順序又は表現から、その問題に適した一部を選び、意味のある変更を加えます。
...
問題文の一部だけを変更しても構いませんが、空白、表記揺れ又は同義語を置き換えただけの変更は完成扱いにしません。一方、AWSサービス名、法令名、専門用語など、正確さのために必要な語句は無理に変更しません。選択肢はすべて維持できます。変更する場合も、一つ又は必要最小限の選択肢にとどめ、正答の決め手、誤答としてのもっともらしさ、要求する判断を変えません。差を作ることだけを目的に書き換えず、変更文字数や類似率の数値基準も設けません。

公開前の機械チェックは、問題文全体又は取得元の解説との完全一致を検出する最小限のものにします。選択肢一式の一致だけでは拒否しません。自然さ、難易度、資格らしさ、問題文と選択肢の整合、正答の妥当性は既存の評価工程で確認します。
