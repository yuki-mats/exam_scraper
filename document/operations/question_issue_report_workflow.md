...
4. `quality-gate`
5. questions upload dry-run
6. correction unit の変更だけを commit / push
7. questions upload
8. upload JSON と live `questions/{questionId}` の field readback

専用 clone は `config/question_issue_reports.json` の `canonicalBranch` にいなければならず、correction unit 開始前に clean かつ同名の `origin` branch と同一であることを確認する。push 先も current branch 任せにせず、その明示 branch に固定する。現在の正本は `codex/goal-driven-workflow`。remote `main` は履歴が大きく遅れているため、この workflow から更新しない。

readback が一致するまで `published` にしない。upload command の終了だけでは完了ではない。commit 後の push/upload/readback 失敗は case を `unreviewed` へ戻さず、commit・upload JSON・originalQuestionId を持つ `publish_pending` にする。元の承認を再利用して次を実行し、同じ correction unit を再レビューせず完了させる。
