...
model turnはread-onlyで`question-maintenance-candidates/v3`候補だけを返し、serverが候補を問題ID、工程、入力fingerprint、projection hash、内容hashとともに一問state内のattemptへwrite-onceで保存します。既存candidate v2は再開互換のため読み取ります。汎用`question-tool` executor上のpatch tool queueはattempt IDだけを持ち、保存済み候補をreadback検証してから`patchApplyStartedAt`をwrite-onceで記録します。旧runの`candidateCommitStartedAt`は再開判定時だけ読み取ります。中断後に同じ候補を再利用できるのは、入力identityが完全一致し、どちらの開始記録もない場合だけです。serverは`attempts/<token>/`へresult、progress、開始前baselineも保存します。patchは工程・年度ごとの既存形式を維持し、一問ごとには分割しません。実際に更新するpatch JSONと`work_versions.json`だけを同じ実path transactionに含め、checkpoint保存まで成功した後に確定します。失敗時は実path lockを解放する前に両方を開始前へ戻します。再起動時はtransactionが開いた一問を開始前へ戻し、確定済みreceiptを持つ問を除外して未完了だけを再開します。詳細は[問題整備システム](local_question_review_console.md)、評価内容は[`evaluation_result.schema.json`](../../tools/question_review_console/evaluation_result.schema.json)を正本とします。
