...
機械品質ゲートの後、[問題整備システム](local_question_review_console.md)で元問題を1問ずつ別セッション評価へ通します。別セッションは、問題文と全選択肢を一体で読み、各選択肢の正誤を根拠付きで確認し、現在の正答対応と解説品質を同じ評価で判定します。
...
評価結果からシステムが`publishReady`を計算し、手動で合格に変更できないようにします。品質確認はpatchやupload-readyを直接変更しません。不合格は01、02、02a、02b、03、03bの該当工程へ戻し、再生成後に同じ問題を新しい別セッションで再評価します。他の問題の不合格は、合格した問題の公開を妨げません。
