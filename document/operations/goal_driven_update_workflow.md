...
法令問題を含む厳密レビューでは、さらに現行法監査を含める。スクレイピング元の正誤は出題当時の公式正答または掲載元の正誤である可能性が高いため、現行法と突き合わせる。明らかに現行法で正誤が変わる場合は、現行法ベースへ `correctChoiceText` / `explanationText` を更新し、更新済みであること、出題当時の正答との差分、参照条項を問題データと review receipt に残す。差分が疑われるだけで確認できない場合は、正誤を変えずに再確認フラグへ送る。
...
6. `03_prompt_add_explanationText.md`
   ...
   - 法令問題では現行法と出題当時法令を突き合わせ、明確な差分があれば現行法ベースへ更新し、更新済み注記と根拠条項を残す
