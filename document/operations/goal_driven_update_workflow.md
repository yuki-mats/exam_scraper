...
これらは、同じ `list_group_id` でも入力正本と品質基準が違う。`explanation` は正しい `questionType`、`questionIntent`、`correctChoiceText`、および02bで整理した `law_context` を前提にするため、最後に回す。

厳密レビューでは、Worker の役割を一般的な目視確認者ではなく、その資格の専門家、問題作成者、参考書著者として扱う。正答を当てるだけでなく、出題意図、根拠、受験者が誤学習しない説明、教材としての分かりやすさまで確認する。

法令問題を含む厳密レビューでは、03の前に02bで現行法根拠候補を整理する。スクレイピング元の正誤は出題当時の公式正答または掲載元の正誤である可能性が高いため、現行法と突き合わせる。明らかに現行法で正誤が変わる場合は、03bを通して現行法ベースへ `correctChoiceText` / `explanationText` を更新し、更新済みであること、出題当時の正答との差分、参照条項を問題データと review receipt に残す。差分が疑われるだけで確認できない場合は、正誤を変えずに再確認フラグへ送る。
...
- 03前の `isLawRelated` / `lawReferences` 事前整理
- `category.json` の新設 / 再編
- 法令問題での条項裏取り
- 法令問題での現行法差分確認と、現行法ベースへ更新するかどうかの判断
