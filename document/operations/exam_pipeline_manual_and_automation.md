...
8. `02a_prompt_review_correctChoiceText.md` で全設問の最終 `correctChoiceText` を一問ずつ見直し、mergeで`20_merged_1/`へ反映する
...
flowchart LR
  ...
  CorrectDraft --> DraftMerged["merge<br/>20_merged_1 正誤下書き"]
  DraftMerged --> CorrectReview["02a 厳密 correctChoiceText review<br/>23_correctChoiceText_fixed"]
  CorrectReview --> Merged1["merge<br/>20_merged_1 に厳密正答反映"]
...
- `20_merged_1/`: `questionType`、`questionIntent`、`answer_result_text`由来の下書きと、02aの厳密`correctChoiceText`レビューを反映した、解説作成前の主入力。
- `21_explanationText_added/`: `explanationText` patch。
- `22_questionSetId_linked/`: `questionSetId` patch。
- `23_correctChoiceText_fixed/`: 02aで03前に作る最終`correctChoiceText`の厳密レビューpatch。後から更新した場合は03も再生成する。
...
| 01/02/02a merge 後 | `20_merged_1/` | `<source_stem>_merged.json` | 03前の主入力。`questionType`、`questionIntent`、厳密レビュー済み`correctChoiceText`、02b法令コンテキストを反映する。 |
...
| 02a 厳密正答レビュー | `23_correctChoiceText_fixed/` | `<source_stem>_merged_correctChoiceText_fixed.json` | 03前に`correctChoiceText`を一問ずつ専門家目線で確認した結果を入れる。後から変更した場合はmerge後に03を再生成する。 |
...
4. merge / correctChoiceText 下書き補完
   ...
   - この下書き補完だけでは完了扱いにしない
5. `correct_choice`
   - prompt: `prompt/02a_prompt_review_correctChoiceText.md`
   - 主入力: 下書き補完済み`20_merged_1/question_*_merged.json`
   - `questionIntent`、`answer_result_text`、選択肢、元解説を1問ずつ突き合わせる
   - 出力: `23_correctChoiceText_fixed/`
6. merge
   - 厳密レビュー済み`correctChoiceText`を`20_merged_1/`へ反映する
7. `law_context`
...
9. `explanation`
   ...
   - 主入力: `20_merged_1/question_*_merged.json`
   - 補助入力: 02aの反映確認時のみ`23_correctChoiceText_fixed/`、出題時正答の追跡時のみ`00_source/`
