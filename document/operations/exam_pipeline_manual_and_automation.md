...
> **① 取得・保護**　`資格・取得設定` → `scrape` → **`00_source`**<br>
> ↓<br>
> **② 整備**　公式過去問は`01〜03` ／ それ以外・混在は`05 独自問題化` → `01〜03`<br>
> ↓<br>
> **③ 資格別の後工程**　`03b 現行法監査（対象のみ）` → `03c category.json（未準備時）` → `04 問題集`<br>
> ↓<br>
> **④ 公開前検査**　`merge / convert` → `quality-gate / upload dry-run` → **評価待ち**<br>
> ↓<br>
> **⑤ 評価・反映**　問題ごとの別sessionで評価 → **合格：Firestore反映** ／ **不合格：再整備して④へ**

## 一問整備の基本仕様

> **一問 = 問題文 + 全選択肢 + 正答 + 解説**

問題整備システムは、最大100問を同時に受け付け、各問を独立して進めます。同じ問では、問題文と全選択肢をまとめて現在工程で確定し、機械チェックと確定保存が終わるまで次工程へ進めません。

| 順序 | 一問の中で繰り返す流れ |
| ---: | --- |
| 1 | 必要工程を決める |
| 2 | 現在工程を専用prompt・ツールで実行する |
| 3 | 工程固有の機械チェックを行う |
| 4 | 合格した結果を確定保存する |
| 5 | 次工程へ進む。後続がなければ一問完了 |

| 判定 | その問の動き | ほかの問 |
| --- | --- | --- |
| 同じ工程で直せる | 必要なfeedbackだけを返して再試行する | そのまま進む |
| 前工程が原因 | その問だけ担当する前工程へ戻す | そのまま進む |
| 自動判断できない | その問だけ`hold`にする | そのまま進む |

- 工程の順序、適用条件、prompt、ツール、更新範囲、機械チェックはworkflow設定で管理し、資格に不要な工程は省く。各工程には確定済みの内容と必要な指示・根拠・feedbackだけを渡す。
- 一問では同時に一工程だけを動かし、重複実行、古い結果の保存、二重保存を防ぐ。資格全体の方針、カテゴリ設計、公開準備など、本当に共通確定が必要な工程だけで待ち合わせる。
- 中断後は確定済み工程を保持して最初の未完了工程から再開する。前工程が変わった場合は影響する後続だけをやり直し、`hold`の問は自動再開しない。
- 性能改善のため、待ち時間、モデル・ツール・機械チェック・保存の処理時間、再試行・保留理由、実同時実行数、工程版、入出力fingerprintを一問・一工程単位で記録する。
