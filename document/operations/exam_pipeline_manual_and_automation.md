...
4. トップの`listGroupId`一覧から対象、更新項目、問題番号範囲を指定する。必要工程は更新項目から自動で決まり、対象を一問単位で確定しながら進める。queueとsessionの境界は[問題整備システム](local_question_review_console.md#一問queueとsession)を正本とする。
