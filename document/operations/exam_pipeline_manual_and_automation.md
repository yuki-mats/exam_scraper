...
9. `03_prompt_add_explanationText.md` で `21_explanationText_added/` を作る
10. `04_prompt_link_questionSetId.md` で `22_questionSetId_linked/` を作る
11. `00_merge_all.py` と `prepare_firestore_upload.py` で `30_merged_2/`、`40_convert/`、`upload_to_firestore/` を更新する
12. 画像を Storage に dry-run / upload する
13. category と questions を Firestore に dry-run / upload する
...
法令問題では、スクレイピング元の正誤は出題当時の公式正答または掲載元の正誤を反映しているものとして扱う。現行法と突き合わせ、明らかに正誤が異なる場合は現行法ベースへ `correctChoiceText` / `explanationText` を更新する。その場合は、更新した事実、出題当時の正答、現行法の根拠条項を `explanationText`、`suggestedQuestions`、`suggestedQuestionDetails`、`lawReferences`、review sidecar に残し、ユーザーにも「現行法に合わせて更新済み」と伝わるようにする。現行 schema で question 直下の正式フラグは未対応のため、正式フラグを upload JSON に混入させるには repaso 側の schema / Firestore rules / UI 更新を先に行う。
...
5. `explanation`
   ...
   - 法令問題: 現行法と出題当時法令を一問ずつ突き合わせる。現行法で正誤が明らかに変わる場合は現行法ベースへ更新し、更新済み注記・出題当時正答との差分・根拠条項を残す。差分が疑われるだけで確定できない場合は正誤を変えず、5.5 high 再確認フラグへ送る。
6. `question_set`
...
法令問題では、掲載元やスクレイピング結果の正誤をそのまま現行法の正誤として扱わない。`20_merged_1`、必要なら `23_correctChoiceText_fixed/` と `00_source/` を読んだうえで、e-Gov、官公庁資料、資格別に認めた公式資料、Lawzilla などで現行法と出題当時法令を確認する。
