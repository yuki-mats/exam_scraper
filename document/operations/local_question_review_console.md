...
- 全LLM backendは一つの`llm_call_concurrency` semaphoreを共有する。run内のmodel executorはUIで選択した`questionConcurrency`を上限とするため、実効同時数は選択値、`question_parallelism`、`llm_call_concurrency`の最小値になる。標準設定では三つを20にそろえ、実行状況は予約数でなくmodel turnの開始・終了通知から数える。
