...
- readOnlyの問題詳細でも、同じ表示field内の文字列を選択すると`この箇所を確認`からその問題限定のreviewを開けます。readOnlyでは類似問題の調査を表示せず、選択したfieldと選択肢だけを初期選択します。範囲外、複数fieldをまたぐ選択、又は空の選択ではtoolbarを開きません。manage modeのその問題・資格内類似reviewの選択肢は従来どおりです。
