...
利用者が開始する評価runは、任意の`元問題1問以上`を対象にします。現在表示中の問題をまとめて選択でき、以前の作業で整備した問題も同じrunへ含められます。
...
2. 選択した各問題は別の新規sessionで評価され、一問の失敗後も残りを継続する。
3. 未評価、不合格、古い評価の問題はFirestoreへ反映できない。
4. 全選択肢の根拠と解説基準をserver側で検証する。
5. 合格した問題は、同じrunの不合格問題に関係なく明示確認後に反映できる。
6. 同じ元問題に属する全Firestore documentをまとめ、一部だけ公開しない。
7. upload成功後もreadback不一致なら完了表示にしない。
8. 問題変更後は評価が自動で古くなり、再評価するまで公開できない。
9. iPhone幅で複数選択、状態、評価結果、主操作が重ならず、safe area外へ隠れない。
