...
- 評価は`questionType`、問題文、全選択肢を一問ずつ独立に判定する。現在の正答は先に渡さず、serverが全肢の結果、正答対応、解説品質、重大指摘を検証する。`true_false`は各選択肢を独立した○×問題として公開するため、元問題の選択指示から正しい肢又は誤った肢の個数を一つへ制限しない。複数の正しい肢又は誤った肢があることだけでは不合格にしない。
...
- `要再整備`で絞り込んだ問題は一覧から複数選択し、既存の問題整備runへまとめて戻す。serverは各問の評価結果を現在の`stateHash`と照合し、`summary`、`criticalIssues`、選択肢別評価、`reworkItems`をその問に必要な工程だけへ検査feedbackとして渡す。選択した各問は新しいworkspace-write threadで一問一model turnとして処理し、通常整備と同じ最大100問の同時実行、問題単位の検査・確定、完了枠への自動補充を使う。評価後に内容が変わった問、評価指摘のない問又は未対応工程を含む問は開始前に停止する。
