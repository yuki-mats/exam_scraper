...
- serverは問題の現在projectionを設定された`question_parallelism`まで準備する。LLM呼出しはbackend種別にかかわらず`llm_call_concurrency`を上限とする。上限はコードへ固定せず、UIもserverが返す設定値から選択肢を組み立てる。patch確定は引き続き問題別に行い、問題処理数を上げても同一fileの更新だけを直列化する。
...
- 全LLM backendは一つの`llm_call_concurrency` semaphoreを共有する。初期値は1であり、問題準備数を将来増やしてもLLM呼出し数とは別に制御する。実行状況は予約数でなくmodel turnの開始・終了通知から数える。
