...
大量の過去問を、元問題1問ずつ次の短いループで整備します。

```text
整備する -> 別セッションで評価する -> 合格ならその問題をFirestoreへ反映
                                  -> 不合格ならその問題を再整備
```

- 資格全体について、どこまで終わり、次に何をするかを表示する。
- 問題文、全選択肢、正誤、解説、根拠を元問題単位で扱う。
- 整備とは別のセッションで、全選択肢の正誤と解説品質を一度に評価する。
...
3つの画面や複数の評価laneには分けません。既存の問題一覧と問題詳細の中に、評価状態、定量値、次の主操作を表示します。
...
一覧は例外を先に並べ、各問題に短い状態badgeを一つ表示します。詳細では評価理由、選択肢別判定、根拠、解説点数、再整備対象を確認できます。通常時に表示するprimary buttonは一つだけです。
...
1回の評価は`元問題1問`だけを扱います。整備に使った会話を再利用せず、新しいprocess又はsessionを起動します。入力には次を固定します。
...
既存のgroup単位公開は保守用APIとして残せますが、問題整備システムの標準操作には使いません。

## API

品質評価と問題単位公開に追加するAPIは次の4つです。既存のinventory、workflow、review、sync、readback、job APIは維持します。

| method | path | 用途 |
| --- | --- | --- |
| `POST` | `/api/questions/<questionId>/evaluation-preview` | 現在内容と評価可否を確認する。 |
| `POST` | `/api/questions/<questionId>/evaluation` | 別セッション評価jobを開始する。 |
...
1. 未評価、不合格、古い評価の問題はFirestoreへ反映できない。
2. 全選択肢の根拠と解説基準をserver側で検証する。
3. 合格した1問は、他の問題の状態に関係なく明示確認後に反映できる。
4. 同じ元問題に属する全Firestore documentをまとめ、一部だけ公開しない。
5. upload成功後もreadback不一致なら完了表示にしない。
6. 問題変更後は評価が自動で古くなり、再評価するまで公開できない。
7. iPhone幅で状態、評価結果、主操作が重ならず、safe area外へ隠れない。
