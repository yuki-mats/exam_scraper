...
- App Server stdout直後と監視projection投入前の両境界は、`agentMessage`と`reasoningSummary`の連続deltaをthread・turn・item・summary index単位でlosslessにまとめる。異なる種類の通知をまたいでまとめず、終端通知より前に確定する。まとめた件数、method別件数、両queue peakをtelemetryへ残し、300枠のburstを通知欠測へ変換しない。disk保存側でも完成した公開streamを同じ単位でまとめ、最終全文と終端順序を維持する。
