...
トップの再読込中は画面全体を操作不可にし、整備状況、実行中の作業、表示中の問題一覧を並行して取得していることを表示します。問題一覧の絞り込み、状況確認の初回取得、実行対象のpreviewには、それぞれ操作箇所に近いローディングを表示します。取得の成功、一部失敗、例外のいずれでも全画面表示を解除し、再読込を再度実行できる状態へ戻します。
