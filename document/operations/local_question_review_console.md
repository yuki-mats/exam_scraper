...
`監査パッチをまとめて修正依頼`は、既存の`lawReferences`、`lawRevisionFacts`、`explanationText`を正本扱いして欠損fieldだけを埋める依頼ではない。Codex用依頼には、各対象選択肢で問題文と選択肢を結合した完全命題を作り、保存済み`apiUrl`/`sourceUrl`又はe-Gov条文本文を開いて一問一肢ずつ目視照合すること、確認不能なら`hold`へ戻すこと、類似問題も一括コピーしないことを明記する。
