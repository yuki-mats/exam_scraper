...
`監査パッチをまとめて修正依頼`は、表示中の1問ではなく選択中のqualification全体を処理する専用依頼とする。Codex用プロンプトには単問の問題文、選択肢、関連ファイル、UI選択詳細を展開せず、review JSONを作業起点として示す。qualification配下の全listGroupIdを棚卸しし、法令監査品質不備がある全問題・全選択肢について、完全命題と保存済み`apiUrl`/`sourceUrl`又はe-Gov条文本文を一問一肢ずつ目視照合する。既存メタデータの一括コピーは禁止し、確認不能なら`hold`へ戻す。
