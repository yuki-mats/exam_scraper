...
| 一問queue | 全item走査後、確定した年度ごとに1回実行 | 自動更新失敗時は管理機能の`出力` |
...
画面は一つのpoll管理でrun、job、進捗を更新し、実行dialog表示中は背景pollを止めます。問題は分野・問題番号とsource上の自然な順序で表示し、processedとvalidatedを分けます。進捗から問題を開く「作業対象を確認」には、問題文・選択肢・正答・解説とpatch適用後の`questionType`を表示します。`suggestedQuestions`と`suggestedQuestionDetails`は質問を重複表示せず、一組の「補足質問と回答」カードへ統合します。両fieldの質問が一致しない場合だけ、差分を警告として併記します。
