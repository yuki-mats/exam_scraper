...
- `パッチ変更を反映`: 選択中の資格・listGroupIdだけを既存`prepare_firestore_upload.py`へ渡し、1回の操作でMerge、Convert、upload-ready生成、upload dry-runを開始する。必須field不足がある場合は開始しない。
...
upload-readyは各documentのアップロード必須fieldに加え、公開・採点の正本であるトップレベルの`correctChoiceText`と`explanationText`を非空で必須とする。`isLawRelated=true`では`lawReferences`、`lawRevisionFacts.auditStatus`、`lawRevisionFacts.evidenceSummary`も必須とする。`lawRevisionFacts.current.correctChoiceText`は現行法監査時点の判定を保存する補助メタデータであり、トップレベルの`correctChoiceText`がある場合はその欠落だけでエラーにしない。必須fieldの欠損はdocument IDとdata pathを付けて全件表示する。差分画面の`fieldなし`も必須性にかかわらず文字列選択で`修正を依頼`に渡せる。同一問題の必須field欠損は`欠損をまとめて修正依頼`から1件のCodex依頼にまとめる。
