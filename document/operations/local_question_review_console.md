...
トップの各年度・フォルダには、`整備・洗い替え`の左に読取専用の`状況を確認`を置きます。タップした`listGroupId`の全問題を開き、問題検索と、`00_source`と正答が異なる問題、問題文から選択肢を取得した問題、計算問題、法令問題の絞り込みを利用できます。条件は併用できます。この画面では人間監査結果を保存せず、編集、評価、Firestore反映の操作を主画面へ出しません。
