...
| Firestore upload | 対象年度・回のupload-readyだけを、明示確認後に既存uploaderで反映 |
| Firestore一括確認 | 1資格内の単一又は複数年度・回を明示選択。全資格一括は行わない |
| 環境切替 | 初期版では実装しない |
...
- `成果物を同期`: 選択中の資格・年度・回だけを既存`prepare_firestore_upload.py`へ渡し、Merge、Convert、upload-ready生成、upload dry-runまで実行する。
...
- `Firestore確認`: 初期状態では現在の年度・回だけを選択する。同じ資格内で複数年度・回又は資格内全年度を選択でき、Firestoreへ接続する前に対象フォルダ数、元問題数、読取対象document数を表示する。
...
- 一括確認でも`questions` collectionを全件走査せず、選択した資格・年度・回のupload-readyに列挙されたdocument IDだけを`get_all`で取得する。
- 一括確認の初期選択は現在の年度・回1件とし、全資格を一度に選択する操作は提供しない。
- 複数年度・回の読取は400 documentごとに分割し、進捗を表示する。
- 読取結果はサーバー実行中のメモリへ資格・年度・回別に保持する。1フォルダの同期又は公開後はそのフォルダだけを`未取得`へ戻し、他資格・他フォルダの結果を破棄しない。
