...
`生成内容を監査`は独立した監査画面を開きます。通常は年度、検索、`要確認のみ / 全問`だけを使い、問題本文、選択肢、patch適用後の正答と解説を一問ずつ見比べます。ズレを見つけた場合はその問題から再整備を依頼します。工程選択、作業履歴、評価、Firestore、関連ファイルなどは監査画面末尾の`工程・評価・Firestoreなどの管理機能`へ畳み、必要な場合だけ開きます。問題一覧は監査画面を開いた時だけ最新状態へ更新し、実行専用画面では読み込みません。
