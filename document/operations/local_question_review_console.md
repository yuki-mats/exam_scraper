...
│ 00取得 → 準備 → 01形式 → 02意図 → 02a正答 → 02b根拠 → 03解説 → │
│ 03b監査 → 04問題集 → 出力                               │
├──────────────┬─────────────────────────────┤
...
資格を選ぶと、全listGroupIdを横断して次の工程を判定する。工程は`00`、資格方針の準備、`01`、`02`、`02a`、`02b`、`03`、`03b`、`04`、出力の順に固定し、各工程には目的、完了数、残数、要確認状態を表示する。
...
`00_source`に値があっても人間確認済みとは見なさず、責務に合うpatchの存在で`01`〜`04`の完了を判定する。`02a`は`23_correctChoiceText_fixed`の全問coverageで判定し、03より前に完了させる。`03b`は法令関連問題だけを対象とし、`02b`完了後に`lawRevisionFacts`と未解決の法令監査issueを確認する。出力工程はlistGroupId単位でmerge、convert、upload-readyの内容一致を確認する。本番Firestoreの一致は出力完了と分離して扱う。
...
- `未作業のみ`: 対応するpatch又は成果物がない対象
- `要確認のみ`: 当該工程のfieldに未解決issueがある対象
- `全件洗い替え`: 既存patchの有無にかかわらず、当該工程の全対象
...
一覧APIとDOM表示は50件単位でページ分割する。資格全体の件数とissue件数は全件集計値を維持し、`さらに表示`で次の50件だけを追加する。資格切替時に数千問を一度にDOMへ展開しない。
...
| `GET` | `/api/qualification-workflow` | 選択資格全体の工程状態、目的、次工程、対象件数 |
| `GET` | `/api/qualification-runs` | 選択資格の直近作業、進捗、中断状態 |
...
| `POST` | `/api/qualification-runs/preview` | 工程と範囲ごとの対象件数、安全境界、実行可否を確認 |
| `POST` | `/api/qualification-runs/start` | 人間工程の依頼保存又は資格単位のローカル出力jobを開始 |
| `POST` | `/api/qualification-runs/resume-prompt` | 保存済みのCodex依頼を再取得 |
...
tests/
  ...
  test_question_review_qualification_runs.py
