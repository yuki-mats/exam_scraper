...
2. トップの年度・フォルダ（`listGroupId`）一覧で`整備・洗い替え`を開き、対象、更新項目、問題番号範囲、対象条件を指定する。工程は更新項目から自動で決まり、serverは範囲を一問queueへ分解して`00_source`と確定patchの論理projectionを次工程へ渡す。
...
- UIで工程を直接選ばない。選択したupdate targetが属する工程をworkflow順で自動実行する。更新項目は初期未選択とし、現行基準で整備が必要な工程の項目をまとめて選ぶ補助操作だけを設ける。
- 問題番号は`examYear`ではなく`listGroupId`ごとの表示順で数える。開始と終了は1始まりの包含範囲で、複数の年度又はフォルダを選んだ場合は、それぞれへ同じ範囲を適用する。独自問題に`examYear`がなくても実行契約は変わらない。
...
- preview tokenとrun receiptには`selectedUpdateTargetIds`、`selectedFieldsByStage`、`readFieldsByStage`、`questionRange`を保存する。再開時に一つでも違う場合は別runとして確認し直す。
...
- GUIの開始範囲は資格・年度又はフォルダ・問題番号・工程・更新項目で指定し、serverが`sourceQuestionKey`、`reviewQuestionId`、`sourceRecordRef`、工程、update targetの組へ分解する。一問だけ残る場合も同じqueueを使う。資格全体で一つだけ持つ方針・03c分類は問題patchではなく共有前提として分離し、失敗時は依存する問題工程だけを保留する。
