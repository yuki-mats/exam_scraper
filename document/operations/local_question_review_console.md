...
patch更新のCodex依頼は、工程に固定された正本文書と、GUIで選んだ`未作業のみ`、`要確認のみ`、`全件洗い替え`の対象pathを組み合わせて生成します。正本文書は安全要件なので対象範囲から外せません。
...
開始前に正本文書数、対象source数、更新先数、対象件数とlistGroupIdをpreviewし、runを次へ保存します。
