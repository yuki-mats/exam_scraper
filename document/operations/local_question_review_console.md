...
この文書は、`tools/question_review_console/`の現行UI・API・書き込み安全性の正本です。問題整備工程は[幹](exam_pipeline_manual_and_automation.md)、patchの責務は[artifact契約](artifact_contract.md)を参照します。
...
表示順は[prompt一覧](../../prompt/README.md)とdelivery工程に従います。

```text
00 → 準備 → 01 → 02 → 02a → 02b → 03 → 03b → 04 → 出力
```
...
`00_source`に値があっても人間確認済みとはみなしません。02aは`23_correctChoiceText_fixed`の全問coverage、03bは法令関連問題の`lawRevisionFacts`と未解決issue、出力はgroup単位のmerge・convert・upload-ready一致で判定します。Firestore一致はローカル出力完了と分離します。

工程開始時の範囲は`未作業のみ`、`要確認のみ`、`全件洗い替え`です。開始前に件数とlistGroupIdをpreviewし、runを次へ保存します。
...
- mutation APIは起動session tokenとlocal Originを必須にする。
