...
トップの再読込では画面全体を操作不可にしません。整備状況、実行中の作業、表示中の問題一覧を並行して取得し、取得できた区画から表示を更新します。更新前の情報は各区画の新しい応答が届くまで残し、再読込ボタンと対象区画の近くにだけ更新中であることを表示します。問題一覧の絞り込み、状況確認の初回取得、実行対象のpreviewにも、それぞれ操作箇所に近いローディングを表示します。一部の取得が失敗しても、取得できた情報の閲覧と再読込を続けられる状態を保ちます。
