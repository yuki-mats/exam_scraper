...
- serverは問題の現在projectionを設定された`question_parallelism`まで準備する。LLM呼出しはbackend種別にかかわらず`llm_call_concurrency`を上限とし、初期値はどちらも1である。上限はコードへ固定せず、UIもserverが返す設定値から選択肢を組み立てる。patch確定は引き続き問題別に行い、将来問題処理数を上げても同一fileの更新だけを直列化する。
