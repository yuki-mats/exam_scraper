...
3. serverは[幹の基本仕様](exam_pipeline_manual_and_automation.md#一問整備の基本仕様)に従い、設定された問題処理数で進行させ、各問を独立して検査・確定する。初期設定は問題処理もLLM呼出しも1並列である。
...
運用の既定profileはcodex_onlyです。候補生成と通常再試行はGPT-5.6 Luna、独立reviewと公開前の問題品質監査はGPT-5.6 SolのCodex App Serverで実行します。監査不合格をSolによる再生成へ流用せず、具体的な検査feedbackを同じLunaの一問queueへ返します。これにより、高頻度の整備費用を抑えながら、候補を作ったmodelとは異なる上位modelで品質を確認します。local_generate_codex_auditは差し替え可能性を検証する実験profileとして残しますが、現在は未承認で運用に使えません。qwen3:14bとqwen3.5:27bは固定7問の実測で品質・速度基準を満たさず、どちらも不採用です。一問の整備完了はおおむね3分以内を受入目安とし、別のmodelを使う場合は改めて実問受入を通します。最終品質はSol監査だけに委ねず、工程固有の機械検査、固定問題集合の回帰評価及び公開前の人間確認を合わせて判定します。
model接続と上限の正本は[`config/question_maintenance_llm.toml`](../../config/question_maintenance_llm.toml)です。`codex_only`は整備・監査ともCodex App Server、`local_generate_codex_audit`は整備をOpenAI互換HTTP、監査をCodex App Serverへ送ります。profileはpreviewとstartで固定し、選択したprimaryとfallbackのkind、endpoint、model、timeout、認証環境変数名を含む公開設定とfingerprintをmanifestへ保存します。API keyの値は環境変数だけから読み、設定・manifest・receiptへ保存しません。UIでは一回のrunの対象を1〜500問で指定でき、既定は100問です。候補が指定数を超える場合は、serverが返した確定対象identityから指定数の問題IDを固定して再previewし、startも同じ問題集合へ結び付けます。現在の標準設定は`question_parallelism=100`、`llm_call_concurrency=100`です。UIで指定できる同時処理数はこの二つの小さい方を上限とし、選択した問題数まで独立したmodel turnを同時に進めます。したがって、500問を対象にしても同時処理は最大100問のローリングwindowであり、対象問題数を増やして同時実行上限を迂回しません。全backendは同じ共通semaphoreに従い、patch確定の安全境界とは分離します。`local_generate_codex_audit`の初期設定は、`qwen3:14b`をprimaryとretryに使い、各600秒で最大2回試した後だけCodex App Serverへfallbackします。primary、retry、fallbackは別pipelineではなく、同じ一問queueのattempt履歴とprofile fingerprintから決定します。network、timeout、HTTP 429、HTTP 5xxだけを再試行できます。安全設定、認証環境変数欠損、HTTP 4xx、応答上限超過、HTTP応答のJSON又はschema不一致、応答model不一致はその問を直ちに保留し、retryもfallbackもしません。各attemptはbackend、要求model、検証済み実model、fallback有無を保存します。ローカル成功へ数えるのは、候補の決定的検査と問題別保存まで完了したattemptだけであり、候補検査失敗とfallback成功は含めません。
...
- serverは問題の現在projectionを設定された`question_parallelism`まで準備する。LLM呼出しはbackend種別にかかわらず`llm_call_concurrency`を共通上限とする。UIと開始APIは両設定の小さい方だけを同時処理上限として提示・許可するため、選択値より低い隠れたmodel上限を作らない。patch確定は引き続き問題別に行い、問題処理数を上げても同一fileの更新だけを直列化する。
...
- 第01工程は、全問題に対して同じsource snapshotを使う独立したread-onlyレビューを2回実行し、serverが結果を照合してから通常の問題形式候補を生成する。これらのレビューと不一致時の裁定は`audit` workとして扱い、ハイブリッドprofileでもCodexへ送る。通常の問題形式候補だけを`maintenance` workとしてローカルLLMへ送る。レビューの詳細schemaはproductionコードを正本とし、この文書には複製しない。予約、二つの結果、照合結果は、親manifest全体へ書き戻さず、親run配下の`aggregate_review_checkpoints/<questionIdのsha256>.json`へ問題単位で保存する。異なる問題の記録は互いのlockを待たず、同じ問題のslotだけを直列化する。二者の判断だけが不一致の場合は、原文、候補、両レビューを渡した別のread-only裁定を一度実行する。裁定で一意に確定できた問題は通常処理へ戻し、裁定でも判定不能又は境界不明の問題、source hash不一致、review証跡不正だけを問題単位の`hold`とする。候補境界は大文字・小文字、全角・半角の英字と、半角又は全角のピリオドを同じmarker familyとして認識し、同一行でも改行区切りでも原文offsetから抽出する。markerだけで記述本文がない場合は候補にせず`hold`を維持する。対象確定時の記述本文はserverが合意済みspanから切り出し、model出力の文章を保存しない。
...
- 問題projectionの準備とmodel候補生成は`question_parallelism`の範囲で進める。projectionを保存した後、問題本文と各選択肢について文字列又は画像のいずれかがあるかを機械確認し、不足があれば構造化issueを一問stateへ保存して、その問題と依存工程をmodel実行前に`hold`へ移す。法令根拠は従来どおり02b・03・03bの候補生成前に取得し、02aへ工程責務を前倒ししない。候補をattemptへ永続化した時点で大きな入力を解放し、patch tool queueにはattempt IDだけを渡す。model候補はread-onlyである。中断runの候補を再利用できるのは、同じ問題・工程・入力fingerprint・projection hashで、かつ`patchApplyStartedAt`がない場合だけとする。一問state、集約回答checkpoint、工程版台帳は問題ごとのlockで扱い、正本patchだけは実際に共有するJSON path単位で直列化する。
- 全LLM backendは一つの`llm_call_concurrency` semaphoreを共有する。run内のmodel executorはUIで選択した`questionConcurrency`を上限とするため、実効同時数は選択値、`question_parallelism`、`llm_call_concurrency`の最小値になる。標準設定では三つを100にそろえ、実行状況は予約数でなくmodel turnの開始・終了通知から数える。
...
- Codex App Serverのturn待機中も60秒間隔で`heartbeatAt`を更新する。親runのheartbeat writerはcoordinator一つだけとし、各一問turnから親manifestへheartbeatを書き込まない。heartbeatはjobの`lastActivityAt`へ伝播するが、問題処理又はreceipt検証の完了を意味しない。
...
- App Server stdout直後と監視projection投入前の両境界は、`agentMessage`と`reasoningSummary`の連続deltaをthread・turn・item・summary index単位でlosslessにまとめる。異なる種類の通知をまたいでまとめず、終端通知より前に確定する。disk保存側でも完成した公開streamを同じ単位でまとめ、最終全文と終端順序を維持する。
...
- `questionBodyText`と`choiceTextList`の選択reviewは`needs_review`の記録専用とし、単独のCodex runを開始しません。serverはこれらのfieldに`startCodex=true`が指定された場合、reviewとpromptを保存する前に拒否します。保留再整備では、対象問題の現在`stateHash`と一致する最新の`current_question`指摘だけをserverが解決し、元runの保留理由とともにpreview tokenへ固定します。問題ごとの最初の再実行工程は、元runの一問queueに記録された工程statusとpolicy fingerprintを現行workflow順で照合して決め、その工程以降だけをqueueへ戻します。決定的validatorが`ownerStageId`付きの構造化issueを返した場合は、その所有工程から後続だけを戻し、message本文から工程を推定しません。別工程へ戻すissueは同じ工程で3回繰り返さず、保留再整備へ引き渡します。policy又は入力が変わっていれば、通過済みでも必要になった最初の工程からやり直します。旧runの構造化されていない保留理由は、同じ工程のfeedback表示と再試行だけに使います。clientから任意のfeedbackは受け付けません。
...
- `要再整備`で絞り込んだ問題は一覧から複数選択し、既存の問題整備runへまとめて戻す。serverは各問の評価結果を現在の`stateHash`と照合し、`summary`、`criticalIssues`、選択肢別評価、`reworkItems`をその問に必要な工程だけへ検査feedbackとして渡す。問題文、選択肢又は成立条件を直さないと正答が一意にならない指摘は05へ渡し、現在の問題文と選択肢を維持した正答対応だけの指摘は02aへ渡す。05を実行する問で正答対応も不一致なら05、02a、03の順に進める。01を再確認する問題は、`questionType`変更に伴う`explanationText`の一問単位・選択肢単位の形を同じturnで確定するため、03も自動で実行する。異なる工程組合せの問題を同じrunへ入れる場合、元の選択工程数と問題ごとに絞った実queue数を別々に保持し、実queueだけを進捗分母にする。選択した各問は通常整備と同じ設定上限を使い、問題単位で検査・確定する。評価後に内容が変わった問、評価指摘のない問又は未対応工程を含む問は開始前に停止する。
...
- 監査turnはCodex App Serverの共通timeoutを使う。batch内でも確定境界は一問単位であり、有効な兄弟問題は他問のschema不正に巻き込まれない。共通timeout等のtransport失敗では同じbatchを単問へ自動展開せず、重複課金と重複runを防ぐ。
