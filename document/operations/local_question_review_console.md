...
トップの再読込中は画面全体を操作不可にし、整備状況、実行中の作業、表示中の問題一覧のどれを取得しているかを段階ごとに表示します。問題一覧の絞り込み、状況確認の初回取得、実行対象のpreviewには、それぞれ操作箇所に近いローディングを表示します。取得の成功、一部失敗、例外のいずれでも全画面表示を解除し、再読込を再度実行できる状態へ戻します。
...
`状況を確認`を開くと、最初の画面にはタップした`listGroupId`の問題一覧だけを表示します。問題をタップしたときに詳細画面へ移り、問題本文、正答、解説、補足質問などを表示します。詳細画面から戻ると検索位置を保った問題一覧へ戻り、一覧取得時に先頭問題を自動表示しません。詳細の取得中は以前の問題を消してローディングを表示し、別の問題に対する操作を誤って実行できない状態にします。問題一覧では、問題検索と、`00_source`と正答が異なる問題、問題文から選択肢を取得した問題、計算問題、法令問題の絞り込みを利用でき、条件は併用できます。この画面では人間監査結果を保存せず、編集、評価、Firestore反映の操作を主画面へ出しません。
