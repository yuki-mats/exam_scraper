...
- トップの各年度・フォルダには、`整備・洗い替え`の左に読取専用の`問題一覧を見る`を置きます。中断又は保留したrunを安全に再開できる場合は、`未完了を再開`も別の操作として表示し、`整備・洗い替え`を隠しません。`未完了を再開`は元runの未完了問題だけを引き継ぎます。その問題の確定済み先行工程で方針fingerprintが変わっていれば、最初に変わった工程からやり直し、完了済みの別問題は対象へ追加しません。`整備・洗い替え`は`resumedFrom`を持たない新しい対象選択を開きます。進捗ゲージは全工程の作業項目を分母にして途中工程の完了も反映し、`整備済み`の問題数は必要な全工程が現行基準になった問題だけを数えます。
