...
- 直近カードより前のrunを再開する場合は、メイン画面の`作業履歴`でrun IDを確認し、その履歴行に表示された`この未完了を再開`を使います。この操作は、安全に再開できる中断又は一部保留runだけに表示し、クリックした履歴runのIDを`resumedFrom`としてpreviewへ渡します。previewで元run ID、対象年度・フォルダ、未完了scope、更新項目を確認してから開始します。
