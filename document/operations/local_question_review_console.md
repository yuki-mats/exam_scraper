...
運用の既定profileはcodex_onlyであり、整備、再試行及び監査をGPT-5.6 LunaのCodex App Serverだけで実行します。品質検査に失敗してもSol、Terra又は別modelへ自動昇格せず、同じLunaへ検査feedbackを返して再試行します。local_generate_codex_auditは差し替え可能性を検証する実験profileとして残しますが、現在は未承認で運用に使えません。qwen3:14bとqwen3.5:27bは固定7問の実測で品質・速度基準を満たさず、どちらも不採用です。一問の整備完了はおおむね3分以内を受入目安とし、別のmodelを使う場合は改めて実問受入を通します。同じLunaの別sessionによるreviewは候補生成から文脈を分離しますが、異種modelによる独立監査ではありません。最終品質は工程固有の機械検査、固定問題集合の回帰評価及び公開前の人間確認を合わせて判定します。
