...
トップの初回表示、資格切替、再読込では、取得中・画面反映中を区別できる待機パネルを表示します。待機パネルには接続確認、整備状況、実行中の作業、必要な場合は問題一覧、画面反映の段階と経過時間を示します。10秒を超えた場合も処理が続いていることを明示し、取得失敗時は表示済みの情報を残したまま再試行できる状態にします。画面全体は操作不可にせず、整備状況、実行中の作業、表示中の問題一覧を並行して取得し、取得できた区画から表示を更新します。更新前の情報は各区画の新しい応答が届くまで残します。問題一覧の絞り込み、状況確認の初回取得、実行対象のpreviewにも、それぞれ操作箇所に近いローディングを表示します。
