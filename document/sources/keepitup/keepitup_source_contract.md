...
- 1問を`question_keepitup-<question_id>.json`へ排他的に新規保存する。既存ファイルは上書きしない。
- 再実行時は一覧を再列挙し、既存IDもliveの問題文、選択肢、正答、解説、画像URLと照合する。差分があれば既存`00_source`を変更せず、取得元内容の競合として停止する。
...
5. 再実行で新規保存が0件、既存内容の照合件数が全件、既存`00_source`のhashが不変である。
