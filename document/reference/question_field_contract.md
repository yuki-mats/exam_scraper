...
`00_source`と整備後の正答差異は、配列indexだけで判定しません。05で選択肢を並べ替えた場合は、選択肢本文の一意な対応を先に確定し、`questionIntent`を使って取得元と現行の「選ばれる選択肢」を求め、その内容が同じかを比較します。用語修正を伴う対応は、同数かつ一対一で、近似候補が一意な場合だけ採用します。対応不能、選択肢数の変更又は選ばれる内容の変更は正答差異として停止し、公式資料をBlind A/B/Challengeで照合した`24_questionIssueCorrections`の正答補正がない限り公開しません。
