...
形式の意味判断は`prompt/01_prompt_fix_questionType.md`だけに置き、候補schemaは型と許可値を定義します。schemaの説明文や取得補修コードに別の分類規則を実装しません。選択肢取得の補修は本文・選択肢・取得元の解答を復元する処理であり、形式を判定する処理ではありません。未分類なら未分類のまま01へ渡します。既存値がある場合も、取得補修がそれを再判定済みと保証するものではありません。
...
| `05_originalized` | patch | `original_question_id`, `questionBodyText`, `choiceTextList`, `correctChoiceText`, `questionIntent`, `answer_result_text` | 取得元の原文を変更せず、独自問題として公開する基礎内容を作る。公式過去問では使わない。05適用後は`answer_result_text`も05を正本とし、legacy 15/23の取得元解答で再上書きしない。`answer_result_inferred_correct_choice_numbers`は`answer_result_text`からmerge時に機械生成する派生値である。 |
...
`00_source`と整備後の正答差異は、配列indexだけで判定しません。05で選択肢を並べ替えた場合は、選択肢本文の一意な対応を先に確定し、`questionIntent`を使って取得元と現行の「選ばれる選択肢」を求め、その内容が同じかを比較します。用語修正を伴う対応は、同数かつ一対一で、近似候補が一意な場合だけ採用します。対応不能、選択肢数の変更又は選ばれる内容の変更は正答差異として停止し、公式資料をBlind A/B/Challengeで照合した`24_questionIssueCorrections`の正答補正又はcurrent-value certificationがない限り公開しません。current-value certificationは`correct_answer`で現在の`correctChoiceText`が既に公式根拠と一致する場合だけ作成し、問題内容を変更せず差異承認だけを記録します。
