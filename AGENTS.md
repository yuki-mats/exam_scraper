...
- 不具合の再発防止を、個別の禁止条件、現在値の固定又は一方への自動補正だけで済ませない。まず、各工程の責務と本来あるべき正常な流れを定義する。
- 複数のfield又は工程が矛盾する場合は、それぞれを正本と内容に基づいて独立に確定し、最後に機械検証で整合性を確認する。機械検証は不整合を検出して止める役割とし、どちらが正しいかを決め打ちしない。
- 禁止条件は、`00_source`保護、ID維持、データ損失防止、外部反映の安全境界など、守るべき不変条件に限る。通常の仕様判断は、禁止事項を増やすより、正しい判定基準と単純な処理順で表現する。
